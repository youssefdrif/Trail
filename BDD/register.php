...
if(isset($_POST['email'],$_POST['nom_utilisateur'],$_POST['prenom_utilisateur'],$_POST['password'])){
...
$url='login.html';
    ...
    if(empty($_POST['nom_utilisateur'])){//le champ pseudo est vide, on arrête l'exécution du script et on affiche un message d'erreur
        echo "<p style='color:red'>Le champ Nom est vide.</p>";
    }  
    elseif(strlen($_POST['nom_utilisateur'])>25){//le pseudo est trop long, il dépasse 25 caractères
        echo "<p style='color:red'>Le Nom est trop long, il dépasse 25 caractères.</p>";
    }    
    elseif(strlen($_POST['prenom_utilisateur'])>25){//le pseudo est trop long, il dépasse 25 caractères
    ...
    elseif (filter_var($_POST['email'], FILTER_VALIDATE_EMAIL) == false ) {
     echo "<p style='color:red'>L'adresse email est invalide.</p>";}
        ...
        if(!mysqli_query($mysqli,"INSERT INTO utilisateur SET email='".$_POST['email']."',nom_utilisateur='".$_POST['nom_utilisateur']."',prenom_utilisateur='".$_POST['prenom_utilisateur']."', password ='".($_POST['password'])."'")){
